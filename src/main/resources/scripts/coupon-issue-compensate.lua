...
local DIAG_NONE = 0
local DIAG_REQUEST_READ = 201
local DIAG_REQUEST_WRITE = 211
local DIAG_STREAM_WRITE = 212
local DIAG_BITMAP_WRITE = 213
local DIAG_STOCK_INCREMENT = 214
...
if numericCode == PERSISTENCE_FAILED and numericLifecycle == COMPENSATED then
    return response(ALREADY_COMPENSATED)
end
if numericCode ~= ACCEPTED or numericLifecycle ~= PENDING then
    return response(NOT_COMPENSABLE)
end
...
if isError(result) then
    ...
    return response(INTERNAL_WRITE_ERROR, DIAG_STREAM_WRITE, errorDetail(result))
end
...
if isError(result) or result ~= 1 then
    rollbackCompensation()
    return response(INTERNAL_WRITE_ERROR, DIAG_BITMAP_WRITE, errorDetail(result))
end
bitmapChanged = true
...
if isError(result) or result ~= restoredStock then
    rollbackCompensation()
    return response(INTERNAL_WRITE_ERROR, DIAG_STOCK_INCREMENT, errorDetail(result))
end

return response(COMPENSATED_NOW)
