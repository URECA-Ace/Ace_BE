...
public enum ErrorCode {
	...
	CAMPAIGN_CLOSE_TEMPORARILY_UNAVAILABLE(
			HttpStatus.SERVICE_UNAVAILABLE,
			"쿠폰 캠페인을 일시적으로 마감할 수 없습니다."),
	...
	// 정합성 스케줄러 주기 관리
	SCHEDULE_NOT_FOUND(HttpStatus.NOT_FOUND, "등록되지 않았거나 비활성화된 스케줄러입니다."),

	// 정합성 위반 데이터 주입(데모/운영 시연용)
	INJECTOR_NOT_FOUND(HttpStatus.NOT_FOUND, "해당 검사 항목에는 위반 주입기가 정의되어 있지 않습니다."),
	INJECTION_TARGET_NOT_FOUND(HttpStatus.CONFLICT, "위반 데이터를 주입할 대상을 찾을 수 없습니다.");
